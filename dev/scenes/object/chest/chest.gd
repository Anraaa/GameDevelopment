...
func _ready() -> void:
	...
	feed_component.food_received.connect(on_received_food)
...
func on_received_food(area: Area2D) -> void:
...
func get_random_position_in_circle(center: Vector2, radius: int) -> Vector2i:
	...
	var disctance_from_center = sqrt(randf()) * radius
	
	var x: int = center.x + disctance_from_center * cos(angle)
	var y: int = center.y + disctance_from_center * cos(angle)
