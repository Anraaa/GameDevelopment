...
func add_tilled_soil_cell() -> void:
	# Naikkan batas jarak jadi 200 atau 300 biar gampang ngetesnya
	if distance < 300.0 and cell_source_id != -1:
		tilled_soil_tilemap_layer.set_cells_terrain_connect([cell_position], terrain_set, terrain, true)
		print("BERHASIL MENCANGKUL!")
	else:
		print("Gagal: Jarak ", distance, " atau Source ID ", cell_source_id)
